...
\begin{definicio}
	A \fogalom{jellemzo} egy modellelemeken értelmezett \fogalom{parcialis-fuggveny}. A jellemzők a modell részei.
\end{definicio}

%\begin{pelda}
Vizsgáljuk meg a jellemzőket \aref{tab:jarmutelepek}. táblában. A modellelemüket a \textit{név} jellemzővel azonosítjuk. A \textit{kapacitás} jellemzőhöz tartozó függvény
$\mathit{kapacitás}(e) \rightarrow h,$
ahol $e$ egy modellelem, $n$ egy nemnegatív egész szám. 
Például
$$\mathit{kapacitás}(\mathsf{Kelenföld}) = 46, \quad \mathit{kapacitás}(\mathsf{Cinkota}) = yy.$$

A \textit{funkció} jellemzőhöz tartozó függvény
$\mathit{funkció}(e) \rightarrow h,$
ahol $e$ egy modellelem, $t$ egy karakterlánc. 
Például
$$\mathit{funkció}(\mathsf{Kelenföld}) = \mathsf{"kocsiszín"}, \quad \mathit{funkció}(\mathsf{Cinkota}) = \mathsf{"autóbuszgarázs"}.$$

A fentiekhez hasonlóan, a \textit{sínpárok száma} jellemzőhöz tartozó függvény 
$\mathit{sínpárok\ száma}(e) \rightarrow n,$
ahol $e$ egy modellelem, $n$ egy egész szám. Fontos különbség viszont, hogy ezt jellemzőt csak bizonyos modellelemekre értelmeztünk, másokra nem: a kocsiszínek esetén vesz fel értéket, az autóbuszgarázsok esetén viszont nem. Vagyis látható, hogy a \textit{sínpárok száma} jellemző csak akkor vesz fel értéket, ha a \textit{funkció} attribútum értéke \textit{kocsiszín}. A \textit{funkció} attribútumot tehát tekinthetjük a telephely \fogalomragozva{tipus}{típusának}. Általánosan:
...
A típusok gráf jellegű modellek esetén is fontosak: ezek határozzák meg, hogy mely elemek között lehet kapcsolat. <- TODO

A \fogalom{tipusgraf} a típusokat ábrázolja. Precízen:
