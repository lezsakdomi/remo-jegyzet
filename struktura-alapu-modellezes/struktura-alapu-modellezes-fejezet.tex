...
\begin{pelda}
A közlekedési vállalat üzemen kívüli járművei telephelyeken parkolnak és itt végzik rajtuk a szükséges karbantartásokat. A metrók és a villamosok telephelye a kocsiszín, a buszoké az autóbuszgarázs.
...
%Az \textsf{autóbuszgarázs} és a \textsf{kocsiszín} a \textsf{telephely} típusnak különböző esetei.
Mindkét típusú telephelynek van \textit{azonosító}, \textit{helyszín}, \textit{funkció} és \textit{kapacitás} attribútuma. Az \textsf{autóbuszgarázs} esetén a \textit{max. üzemanyag}, a \textsf{kocsiszín} esetében a \textit{vágányhossz} attribútumot rögzítjük.

\subsection{Típusok}

\Aref{tab:jarmutelepek}. táblázat példájában láthattuk, hogy a \textit{funkció} jellemzővel megkülönböztethetjük az egyes a telephelyeket. Ha a \textit{funkció} jellemzőt a modellelemek \fogalomragozva{tipus}{típusának} tekintjük, akkor úgy is fogalmazhatunk, hogy a \textit{vágányhossz} jellemző csak a \textsf{kocsiszín} típusú elemekre értelmezett, a \textit{max. üzemanyag} jellemző csak az \textsf{autóbuszgarázs} típusra. A típusokat felhasználhatjuk a gráfok jellemzésére is.

\begin{pelda}
	A korábbi metróhálózatot szeretnénk kiegészíteni a buszhálózatra vonatkozó információval. Szeretnénk tárolni azt is, hogy az egyes vonalakon közlekedő járművek melyik telephelyen parkolnak.
\end{pelda}

\added[id=szarnyas]{Ide egy példa nagyobb jön}

Szeretnénk tárolni, hogy melyik vonalhoz melyik telephely tartozik. Értelemszerűen az autóbuszgarázs autóbuszvonalhoz tartozik, metró járműtelep metróhoz stb.

\remofigscaleframed{busz-metro-telephelyek}{Közlekedési vonalak és telephelyek}{\yedscale}

A típusgráfja:

\remofigscaleframed{busz-metro-telephelyek-tipusgraf}{Közlekedési vonalak és telephelyek típusgráfja}{\yedscale}

A típusgráf és a példánygráf viszonya.

\remofigscaleframed{busz-metro-telephelyek-tipusgraffal}{Közlekedési vonalak és telephelyek példány- és típusgráfja}{\yedscale}

Típushierarchia.

\remofigscaleframed{busz-metro-telephelyek-tipushierarchia}{Közlekedési vonalak és telephelyek típushierarchiája}{\yedscale}

Metamodell.

\remofigscaleframed{busz-metro-telephelyek-metamodell}{Közlekedési vonalak és telephelyek metamodellje}{\yedscale}

Példány és metamodell.

\remofigscaleframed{busz-metro-telephelyek-peldanymodell-metamodell}{Közlekedési vonalak és telephelyek példánymodellje és metamodellje}{\yedscale}
...
\remofigscaleframed{jarmutelepek}{A közlekedési hálózatunk metamodellje}{\yedscale}
...
Például
$$\mathit{kapacitás}(\mathsf{T2}) = 46, \quad \mathit{kapacitás}(\mathsf{T3}) = 265.$$

A \textit{funkció} jellemzőhöz tartozó függvény
$\mathit{funkció}(e) \rightarrow t,$
ahol $e$ egy modellelem azonosítója, $t$ a $\{\mathsf{kocsiszín}, \mathsf{autóbuszgarázs}\}$ halmaz eleme.
Például
$$\mathit{funkció}(\mathsf{T2}) = \mathsf{kocsiszín}, \quad \mathit{funkció}(\mathsf{T3}) = \mathsf{autóbuszgarázs}.$$

A fentiekhez hasonlóan, a \textit{vagányhossz} jellemzőhöz tartozó parciális függvény 
$\mathit{vagányhossz}(e) \rightarrow n,$
ahol $e$ egy modellelem azonosítója, $n$ egy nemnegatív egész szám. Fontos különbség viszont az eddigiekhez képest, hogy ezt jellemzőt csak bizonyos modellelemekre értelmeztünk, másokra nem: a kocsiszínek esetén vesz fel értéket, az autóbuszgarázsok esetén viszont nem. Vagyis látható, hogy a \textit{vagányhossz} jellemző csak akkor vesz fel értéket, ha a \textit{funkció} attribútum értéke \textsf{kocsiszín}.
...
\begin{pelda}
	Szeretnénk megtudni, hogy mely kocsiszín alkalmas legalább 100 jármű befogására.
\end{pelda}

%Ehhez a feltételeket összeköthetjük a ,,logikai és'' %($\wedge$)
%operátorral:
%$$\sigma_{\mathsf{kapacitás} \,\geq\, 100 \,\wedge\,  \mathsf{típus} \,=\, \textsf{'kocsiszín'}} (\textsf{telephely})$$	

Végezzünk szűrést azokra a modellelemekre, ahol a \textit{funkció} attribútum értéke \textsf{kocsiszín}, a \textit{kapacitás} attribútum értéke nagyobb vagy egyenlő 100-nál.
...
\begin{table}[H]
	...
	\begin{tabular}{rllrrrr}
		\toprule
		\it azonosító & \it helyszín & \it funkció & \it kapacitás & \it vágányhossz & \it max. üzemanyag &  \\ \midrule
		           T1 & Angyalföld   & kocsiszín   &           147 &          4922 m &                    &  \\ \bottomrule
	\end{tabular}
\end{table}
...
\begin{table}[H]
	...
	\begin{tabular}{rll}
		...
		Angyalföld   & kocsiszín      &           147  \\
		Budafok      & kocsiszín      &            46  \\
		Cinkota      & autóbuszgarázs &           265  \\
		Kelenföld    & kocsiszín      &            42  \\
...
\remofigscaleframed{halozat}{Hálózat}{0.35}
