...
\begin{definicio}
	A \fogalom{jellemzo} egy modellelemeken értelmezett \fogalom{parcialis-fuggveny}, amely a modell része.
\end{definicio}

\begin{pelda}

Vizsgáljuk meg a jellemzőket \aref{tab:bkk-jarmutelepek}. táblában. A \textsf{név} jellemzőhöz tartozó függvény
$$\mathsf{név}(e) \rightarrow h,$$
ahol $e$ egy modellelem, $h$ egy karaktersorozat. 
Például
$$\mathsf{név}(\mathsf{}) = xx, \quad \mathsf{név}(\mathsf{}) = yy$$

A táblázatban ez a függvény minden elemre értelmezett.

A \textsf{sínpárok száma} jellemzőhöz tartozó függvény:
$$\mathsf{sínpárok\ száma}(e) \rightarrow n,$$
ahol $e$ egy modellelem, $n$ egy egész szám.

Látható, hogy ezt jellemzőt csak bizonyos modellelemekre értelmeztünk, másokra nem:

\end{pelda}

Korábban láttuk, hogy a típusok meghatározzák, hogy az egyes modellelemek milyen kapcsolatban lehetnek egymással és milyen tulajdonságokkal jellemezhetjük őket.
...
Olyan szűrést végzünk a modellen, ami csak azokat a csomópontokat tartja meg, amelyekhez tartozik olyan él, amelynek a címkéje M2 vagy M4. \ref{fig:budapest-metrohalozat-nagykorut-elcimkezett-szurt}

\begin{pelda}
	Szeretnénk megtudni, hogy mely szomszédos metrómegállók között hosszabb egy percnél a menetidő.	
\end{pelda}

A modellen olyan szűrést végzünk, ami csak azokat az éleket tartja meg, amelyek élsúlya 1-nél nagyobb. \ref{fig:budapest-metrohalozat-nagykorut-elsulyozott-szurt}

\begin{figure}[H]
	\begin{subfigure}[b]{0.5\textwidth}
		\centering
		\includegraphics[scale=\yedscale]{budapest-metrohalozat-nagykorut-elcimkezett-szurt}
		\caption{? szűrt részgráf}
		\label{fig:budapest-metrohalozat-nagykorut-elcimkezett-szurt}
	\end{subfigure}
	~
	\begin{subfigure}[b]{0.5\textwidth}
		\centering
		\includegraphics[scale=\yedscale]{budapest-metrohalozat-nagykorut-elsulyozott-szurt}
		\caption{Élsúlyra szűrt részgráf}
		\label{fig:budapest-metrohalozat-nagykorut-elsulyozott-szurt}
	\end{subfigure}
	\caption{A metróhálózatot ábrázoló gráf kiterjesztései.}\label{fig:animals}
\end{figure}
