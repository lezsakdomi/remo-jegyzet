...
Egy modellelem tulajdonságait \fogalomragozva{jellemzo}{jellemzőkkel} vagy más néven \fogalomragozva{attributum}{attribútumokkal} írjuk le. (Gráf modellek esetén a modellelemek lehetnek a modell csomópontjai és élei is.)

Különböző modellelemekre az egyes jellemzők más-más értéket vehetnek fel (de nem szükségszerűen térnek el). A jellemzők nem is feltétlenül értelmezettek minden modellelemre (ún. parciális függvényként írhatóak le a matematika nyelvén).
...
Látható, hogy a táblázatnak nem minden celláját töltöttük ki, mivel az egyes jellemzők nincsenek mindenhol értelmezve. Ezeket a cellákat gyakran az \rovidites{NA} (\roviditesangolul{NA}) rövidítéssel vagy %(különösen az adatbázisokban)
\textsf{null} értékkel jellemezzük.

\subsubsection{Műveletek}

A relációkon most két alapműveletet definiálunk, ezek a \fogalom{szures} (relációalgebrában \fogalom{szelekcio}) és a \fogalom{vetites} (relációalgebrában \fogalom{projekcio}). Ezekkel a műveletekkel a modellünknek egy-egy \fogalomragozva{nezet}{nézetét} állíthatjuk elő.
