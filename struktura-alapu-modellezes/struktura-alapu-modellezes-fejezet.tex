...
\begin{megjegyzes}
	...
	A tartalmazási hierarchia ábrázolható a bennfoglalási viszonyok \emph{implicit megjelenítésével} is. Ebben az esetben egy olyan diagramot rajzolunk, amelyben az egyes síkidomok közötti bennfoglaló ábrázolása reprezentálja a modellben szereplő tartalmazási viszonyt.
	
	\begin{center}
		\includegraphics[scale=0.45]{explicit-tartalmazas}
	\end{center}
...
\begin{definicio}
	A \fogalom{strukturalis-modell} a rendszer felépítésére (\fogalomragozva{struktura}{struktúrájára}) vonatkozó tudás. A strukturális modell az egyes \fogalomragozva{modellelem}{modellelemekek} közötti kapcsolatokat és típusokat definiálja. Az egyes modellelemek \fogalomragozva{egyedi}{egyediek}.
\end{definicio}

a strukturális modell aspektusa: fizikai (felépíts), logikai (funkció)
...
\subsubsection{Tulajdonságmodellek}
...
Vizsgáljuk meg a jellemzőket \aref{tab:jarmutelepek}. táblában.
...
Mivel az egyes telephely modellelemek helyszíne egyedi (nincs két telephely azonos helyen), ezért a továbbiakban a \textit{helyszín} jellemző értékének megnevezésével hivatkozunk az egyes modellelemekre. A \textit{kapacitás} jellemzőhöz tartozó függvény
$\mathit{kapacitás}(e) \rightarrow n,$
ahol $e$ egy modellelem, $n$ egy nemnegatív egész szám. 
Például
$$\mathit{kapacitás}(\mathsf{Budafok}) = 46, \quad \mathit{kapacitás}(\mathsf{Óbuda}) = 265.$$

A \textit{funkció} jellemzőhöz tartozó függvény
$\mathit{funkció}(e) \rightarrow t,$
ahol $e$ egy modellelem, $t$ egy a $\{\mathsf{kocsiszín}, \mathsf{autóbuszgarázs}\}$ halmaz eleme.
Például
$$\mathit{funkció}(\mathsf{Budafok}) = \mathsf{kocsiszín}, \quad \mathit{funkció}(\mathsf{Óbuda}) = \mathsf{autóbuszgarázs}.$$

A fentiekhez hasonlóan, a \textit{vagányhossz} jellemzőhöz tartozó parciális függvény 
$\mathit{vagányhossz}(e) \rightarrow n,$
ahol $e$ egy modellelem, $n$ egy nemnegatív egész szám. Fontos különbség viszont, hogy ezt jellemzőt csak bizonyos modellelemekre értelmeztünk, másokra nem: a kocsiszínek esetén vesz fel értéket, az autóbuszgarázsok esetén viszont nem. Vagyis látható, hogy a \textit{vagányhossz} jellemző csak akkor vesz fel értéket, ha a \textit{funkció} attribútum értéke \textsf{kocsiszín}.

Ha tehát a \textit{funkció} attribútumot a telephely \fogalomragozva{tipus}{típusának} tekintjük, akkor úgy is fogalmazhatunk, hogy a \textit{vágányhossz} jellemző csak a \textsf{kocsiszín} típusú elemekre értelmezett. Általánosan:
...
A modellünkben két típus van: az \textsf{autóbuszgarázs} és a \textsf{kocsiszín}. Például az Óbudai telephely az \textsf{autóbuszgarázs}, míg a Budafoki telephely a \textsf{kocsiszín} típus példánya.
...
\subsubsection{Gráfmodellek}

A típusok gráf jellegű modellek esetén is fontosak. A gráfmodell elemeit (külön-külön a csomópontokat és az éleket is) típusokba sorolhatjuk. A típusok meghatározzák, hogy az egyes csomópontok milyen élekkel köthetők össze.
...
\remofigscale{tipusgraf}{A fogalmak típusgráfja}{\yedscale}
...
\begin{definicio}
	Egy rendszer \fogalomragozva{metamodell}{metamodellje} tartalmazza a típusgráfot, az egyes típusok közötti kapcsolatokat. (Például láthatósági kényszerek, műveletek, származtatott tulajdonságok, multiplicitás, tartalmazás).
\end{definicio}
...
A hierarchiát reprezentáló modellek körmentes gráfot alkotnak.
...
A tulajdonságokat gyakran ábrázoljuk táblázatos formában.

\begin{definicio}
A \fogalom{tulajdonsagmodell} a modellelemeket és azok jellemzőit tartalmazza.
\end{definicio}

A tulajdonságmodellt gyakran táblázatos formában ábrázoljuk. A modellelemek egyedisége feltétele miatt a táblázatnak nem lehet két azonos sora.
...
A struktúramodellekből különböző \fogalomragozva{nezet}{nézeteket} állíthatunk elő. Idézzük fel \az+\refstruc{tab:jarmutelepek} járműtelepeket tartalmazó tulajdonságmodelljét.

\begin{table}[H]
	\centering
	\input{struktura-alapu-modellezes/jarmutelepek}
	%\caption{A BKK járműtelepei (részleges modell).}
	%\label{tab:bkk-jarmutelepek}
\end{table}


\subsection{Műveletek}

A leggyakrabban használt műveletek a \fogalom{szures} és a \fogalom{vetites}. Ezek során úgy \fogalomragozva{absztrakcio}{absztraháljuk} a modellt, hogy bizonyos modellelemeket és/vagy azok egyes jellemzőit elhagyjuk.

\subsubsection{Szűrt nézet}
...
\begin{pelda}
	Szeretnénk megtudni, hogy mely járműtelep alkalmas legalább 100 jármű befogására.
\end{pelda}

Azokra az elemekre szűrünk, amelyeknél a \textsf{kapacitás} jellemző értéke 100-nál nagyobb vagy egyenlő.
$$\sigma_{\mathsf{kapacitás} \,\geq\, 100} (\textsf{járműtelep})$$
...
\begin{table}[H]
	...
	\begin{tabular}{llrrrr}
		\toprule
		helyszín    & funkció           & kapacitás & sínpárok száma & max. üzemanyag \\ \midrule
...
\begin{pelda}
	Szeretnénk megtudni, hogy mely kocsiszín alkalmas legalább xx jármű befogására.
\end{pelda}

%Ehhez a feltételeket összeköthetjük a ,,logikai és'' %($\wedge$)
%operátorral:
%$$\sigma_{\mathsf{kapacitás} \,\geq\, 100 \,\wedge\,  \mathsf{típus} \,=\, \textsf{'kocsiszín'}} (\textsf{járműtelep})$$	

Végezzünk szűrést azokra a modellelemekre, ahol a \textit{típus} típus attribútum értéke \textsf{kocsiszín}, a \textit{kapacitás} attribútum értéke nagyobb vagy egyenlő xx-nél.
...
\begin{table}[H]
	...
	\begin{tabular}{llrrrr}
		\toprule
		\it helyszín    & \it funkció           & \it kapacitás & \it sínpárok száma & \it max. üzemanyag \\ \midrule
...
Olyan szűrést végzünk a modellen, ami csak azokat a csomópontokat tartja meg, amelyekhez tartozik olyan él, amelynek a címkéje M2 vagy M4. \ref{fig:metrohalozat-nagykorut-elcimkezett-szurt}

\begin{pelda}
	Szeretnénk megtudni, hogy mely szomszédos metrómegállók között hosszabb egy percnél a menetidő.	
\end{pelda}

A modellen olyan szűrést végzünk, ami csak azokat az éleket tartja meg, amelyek élsúlya 1-nél nagyobb. \ref{fig:metrohalozat-nagykorut-elsulyozott-szurt}

\begin{figure}[H]
	\begin{subfigure}[b]{0.5\textwidth}
		\centering
		\includegraphics[scale=\yedscale]{metrohalozat-nagykorut-elcimkezett-szurt}
		\caption{? szűrt részgráf}
	...
	\begin{subfigure}[b]{0.5\textwidth}
		...
		\caption{Élsúlyra szűrt részgráf}
...
\begin{pelda}
	Olyan kimutatásra van szükségünk, ami csak az egyes járműtelepek helyszínét és kapacitását tartalmazza.
%	$$\pi_{\textsf{helyszín, kapacitás}} (\textsf{járműtelep})$$
\end{pelda}

Végezzünk szűrést a tulajdonságmodelleken a \textit{helyszín} és a \textit{kapacitás} attribútumokra.

\begin{table}[H]
	\centering
	\begin{tabular}{lr}
		\toprule
		helyszín    & kapacitás   \\ \midrule
		Fehér út    &        xx   \\
		Kelenföld   &       100   \\
		Budafok     &        50   \\
		Cinkota     &       100   \\
...
\remofigscaleframed{halozat}{Hálózat}{0.3}
